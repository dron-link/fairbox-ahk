...
/*
 ...
 - made a function that deals the task of getting the nerfed coordinates for timing based nerfs. "timingBasedNerf"
...
yDeadzoneTrackAndFlag(aX, aY) {
  ...
  if (aY > ANALOG_DEAD_MAX) {
    if (analogHistory[1].y <= ANALOG_DEAD_MAX) { ; if the entry before current does not go above y deadzone
      upY := true
  ...
  if (aY < ANALOG_DEAD_MIN) {
    if (analogHistory[1].y >= ANALOG_DEAD_MIN) {
      downY := true
...
updateAnalogHistory(aX, aY) {
  ...
  if (analogHistory[1].x != aX or analogHistory[1].y != aY) {
    ; save current analog stick position info in history
    newAnalogHistoryEntry := new analogHistoryEntry
    newAnalogHistoryEntry.x := aX
    newAnalogHistoryEntry.y := aY
    newAnalogHistoryEntry.timestamp := currentTimeMS
    newAnalogHistoryEntry.simultaneousFinish := 0
    analogHistory.Pop(), analogHistory.InsertAt(1, newAnalogHistoryEntry)
  }
...
limitOutputs(rawCoords) {
  ...
  global ZONE_CENTER
  global STOOD_UP
  static dashZone := new dashZoneObjectTemplate
  static pivot := new pivotObjectTemplate
  static crouchZone := new crouchZoneObjectTemplate
  static uncrouch := new uncrouchObjectTemplate
  currentTimeMS := A_TickCount

  savePivotHistory(dashZone, pivot)
  saveUncrouchHistory(crouchZone, uncrouch)
  ...
  if ((currentTimeMS - pivot.jump2F.timestamp < JUMP_TIME and pivot.jump2F.force and !pivot.queued.did)
    or (currentTimeMS - uncrouch.jump2F.timestamp < JUMP_TIME and uncrouch.jump2F.force and !uncrouch.queued.did)) {
    ; force output to keep the last coordinate (a jump)
    limitedOutput.x := analogHistory[1].x,  limitedOutput.y := analogHistory[1].y

  } else { ; process the player input and converts it into legal output

    limitedOutput.x := rawCoords[xComp],  limitedOutput.y := rawCoords[yComp]
    processed := reverseNeutralBNerf(limitedOutput.x, limitedOutput.y)
    limitedOutput.x := processed[xComp],  limitedOutput.y := processed[yComp]


    ; updates upY, downY, upYTimestamp, downYTimestamp
    yDeadzoneTrackAndFlag(limitedOutput.x, limitedOutput.y)

    timingBasedNerf(limitedOutput.x, limitedOutput.y, pivot, dashZone, TIMELIMIT_PIVOTTILT, pivotInfo)    
    timingBasedNerf(limitedOutput.x, limitedOutput.y, uncrouch, crouchZone, TIMELIMIT_DOWNUP, uncrouchInfo)
    if pivot.nerfWasCalc {  ; 
      limitedOutput.x := pivot.nerfedCoords[xComp]
      limitedOutput.y := pivot.nerfedCoords[yComp]
    }
    if uncrouch.nerfWasCalc { 
      limitedOutput.x := uncrouch.nerfedCoords[xComp]
      limitedOutput.y := uncrouch.nerfedCoords[yComp]
    }
    
    
    ; fuzz the y when x is +1.00 or -1.00
    limitedOutput.y := getFuzzyHorizontal100(limitedOutput.x, limitedOutput.y
      , analogHistory[1].x, analogHistory[1].y)

    ; if the detected pivot will be passed to the game, record it as "queued"
    ; handles the case of nerfing the "neutral" of a pivot into a dash, so it damages the successful pivot input
    if pivot.wasLookedFor {
      if pivot.fromDetector.did {
        if (dashZone.zoneOf(limitedOutput.x, "") == NOT_DASH) {
          pivot.queued := pivot.fromDetector
        } else {
          pivot.queued.did := false
        }
      } else {
        pivot.queued.did := false
      }
    ...
    ; if the detected uncrouch will be passed to the game, record it as "queued"
    ; handles the case of nerfing the uncrouch input into a crouch, so it damages the successful uncrouch input
    if uncrouch.wasLookedFor {
      if uncrouch.fromDetector.did {
        if (crouchZone.zoneOf("", limitedOutput.y) == STOOD_UP) {
          uncrouch.queued := uncrouch.fromDetector
        } else {
          uncrouch.queued.did := false
        }
      } else {
        uncrouch.queued.did := false
      }
  ...
  rememberSDIZonesNotSaved(limitedOutput.x, limitedOutput.y)

  rememberDashZonesNotSaved(limitedOutput.x, dashZone)

  rememberCrouchZonesNotSaved(limitedOutput.y, crouchZone)
  ; memorizes realtime leftstick coordinates passed to the game
  updateAnalogHistory(limitedOutput.x, limitedOutput.y)
