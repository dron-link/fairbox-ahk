...
/* hello.
 ...
 took matters in your hands and made a more powerful, stable, readable version of this script. 
 I am not only trying to fill a gap in existing keyboard solutions,
 I made this with the hope that this script contains any useful ideas for you.
 ...
 this project is proof of concept of how can we implement a variety of analog stick nerfs. 

 everything is subject to modification and may not be present in the finalized version.
 +++ considering making a faithful b0xx v4.1-like in the future.
 ...
 - allow for c-stick button leftstick angle modifier binding changes (or c-stick-angle-bindings for short) 
    by editing an autogenerated file
...
setTimer fire polling rate is apparently 15.6ms, don't expect much precision from it. 
(At least it's shorter than a game cube input polling interval)
...
/*
  ...
  When scanning for techniques that need to be watched out for nerfs,
    ...
    People virtually never press two buttons at the exact same time even when they intend to, 
      and either Autohotkey, the OS or the keyboard can be arbitrary with how they handle "simultaneous" inputs
      and the time gap between them.
    With our input processing method, any input undone before this timelimit settles 
      won't count when checking history for technique detection.
    With this, we mainly want to factor out inconsistencies between different keyboard brands, inconsistencies that appear in gaming.
    We perceive that this makes the detection of techniques more accurate.

  Pivot detection. (empty pivots) ( Adapted from CarVac 's work )
  The detector trips when you input a dash, 
    and before entering run state you terminate your dash by inputting another dash in the opposite direction for about 1 frame.
...
ANALOG_HISTORY_LENGTH := 1 ; MINIMUM 1
...
; analog history simultFinal bits
...
for index, element in cStickAngling {
  ...
  if (cButtonBringsAngle%element% = "cDown" or cButtonBringsAngle%element% = "c-down" 
    or cButtonBringsAngle%element% = "c down") {
    cButtonBringsAngle%element% := "cDown"
    cIniCompleteness |= 1
  } else if (cButtonBringsAngle%element% = "cLeft" or cButtonBringsAngle%element% = "c-left" 
    or cButtonBringsAngle%element% = "c left") {
    cButtonBringsAngle%element% := "cLeft"
    cIniCompleteness |= 1<<1
  } else if (cButtonBringsAngle%element% = "cUp" or cButtonBringsAngle%element% = "c-up" 
    or cButtonBringsAngle%element% = "c up") {
    cButtonBringsAngle%element% := "cUp"
    cIniCompleteness |= 1<<2
  } else if (cButtonBringsAngle%element% = "cRight" or cButtonBringsAngle%element% = "c-right" 
    or cButtonBringsAngle%element% = "c right") {
    cButtonBringsAngle%element% := "cRight"
...
for index, element in cStickAngling {
  if (cButtonBringsAngle%element% = "cDown") {
  ...
  } else if (cButtonBringsAngle%element% = "cLeft") {
  ...
  } else if (cButtonBringsAngle%element% = "cUp") {
  ...
  } else if (cButtonBringsAngle%element% = "cRight") {
...
getFuzzyHorizontal100(outputX, outputY, historyX, historyY) {   ; if you input +/- 1.0, that value may be passed to the game
  global ANALOG_STICK_MIN                                       ; as +/- 0.9875 for as long as you hold the stick in the same place
