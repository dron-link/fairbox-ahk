...
Note: if even with Special Bind active, you can't bind Tab, Back, or Space,  and you don't receive an alert that your chosen key is already used, there's a good chance this program just refused to alert you of it. Look closer for any button that already uses that key, then clear that key.

Note: After you're done, check if all of your keybindings work.

Note: If two different key bindings appear here with the same name, they may still work as two different keys, but to know for sure, check it.

Note: even if a key binding shows here as an invisible character, it may still work.

Tip: You can reopen the program by right-clicking the tray and selecting ''Reload this Script'' or with the key combination Ctrl+Alt+R.

Tip: to restore defaults, close this program, go to the folder containing your program ''fairbox'' and delete the file named ''hotkeys.ini'' once.
)"

    yOff += 30
    
    guiFontDefault("controlsWindow")
    Gui, controlsWindow:Add, GroupBox, xm+%xOff% ym+%yOff% w475 r27, % "Instructions" ; options r%n% means n rows max allocated
    guiFontContent("controlsWindow")
    Gui, controlsWindow:Add, Text, xp+15 yp+25 +Wrap w450, % instructionsText1
    guiFontDefault("controlsWindow")
    Gui, controlsWindow:Add, Button, w250 gTurnOffHotkeysMessage, % "How to use marked keys normally again"
    guiFontContent("controlsWindow")
    Gui, controlsWindow:Add, Text, xp yp+42 +Wrap w450, % instructionsText2   
    return
}
